...
class EventoControllerTest {
    ...
    @Test
    @DisplayName("POST /api/eventos - Crear Evento responde 201 Created")
    ...
    @Test
    @DisplayName("GET /api/eventos/buscar - Buscar Eventos responde 200 OK")
    void testBuscarEventos() throws Exception {
    ...
    @Test
    @DisplayName("POST /api/eventos/{id}/comprar - Comprar entrada guarda compra y responde 200 OK")
    void testComprarEvento_Exitoso() throws Exception {
        ...
        when(eventoService.comprarEvento(eq(1L), eq("usuario@upm.es"))).thenReturn(compraResponse);

        mockMvc.perform(post("/api/eventos/1/comprar")
                        .param("email", "usuario@upm.es"))
                .andExpect(status().isOk())
